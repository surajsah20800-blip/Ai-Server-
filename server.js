import express from "express";
  ...
  app.get("/", (_, res) => res.json({ status: "ok", message: "Gemini AI Server Live! 🚀", keySet: !!KEY }));
  ...
  app.post("/api/gemini/chat", async (req, res) => {
    if (!KEY) {
      return res.status(500).json({ error: "GEMINI_API_KEY not set in Railway Variables! Please add it." });
    }
  ...
  app.listen(PORT, "0.0.0.0", () => {
    ...
    if (!KEY) console.warn("⚠️  WARNING: GEMINI_API_KEY not set! Add it in Railway Variables.");
  });
